...
async function fetchExpenses() {
  ...
  try {
    ...
    showToast("⚠️", "Could not connect to server. Check API_BASE URL.");
    expenses = [];
    renderAll();
  } finally {
    showLoading(false);
    ...
    <div class="expense-card" data-id="${e._id}" style="animation-delay:${i * 40}ms">
      <div class="flex items-start justify-between gap-3">
        <div class="flex items-start gap-3 min-w-0">
          ...
          <div class="min-w-0">
            ...
            <div class="flex items-center gap-2 mt-1 flex-wrap">
              ...
              <span class="text-xs text-white/30" style="font-family:'JetBrains Mono',monospace">
                ${formatDate(e.date)}
              </span>
            </div>
          </div>
        ...
        <div class="flex items-center gap-2 flex-shrink-0">
          <span class="text-base font-bold text-white" style="font-family:'JetBrains Mono',monospace">
            ₹${formatNum(e.amount)}
          </span>
    ...
    options: {
      ...
      scales: {
        x: {
          ...
          ticks: { color: "rgba(255,255,255,0.4)", font: { size: 11, family: "Space Grotesk" } }
        },
        y: {
          grid: { color: "rgba(255,255,255,0.04)" },
          ticks: {
